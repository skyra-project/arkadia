...
public class PubSubClient : IPubSubClient
{
	...
	public PubSubClient(RequestCache cache, HttpClient httpClient, ILogger<PubSubClient> logger)
	{
		...
		_callbackUrl = Environment.GetEnvironmentVariable("CALLBACK_URL") ??
		               throw new ArgumentException("The environement variable 'CALLBACK_URL' must be set.");
	}
	...
	private async Task<Result> SendRequestAsync(string channelId, bool isSubscription)
	{
		...
		_logger.LogError("Subscription request to pubsubhubbub failed: {Error}",
	...
	private FormUrlEncodedContent BuildForm(string channelId, bool isSubscription)
	{
		...
		collection.Add(new KeyValuePair<string?, string?>("hub.topic",
