...
public class SubscriptionManager
{
	...
	public SubscriptionManager(IPubSubClient pubSubClient, ILogger<SubscriptionManager> logger,
		IYoutubeRepositoryFactory repositoryFactory, IChannelInfoRepository channelInfoRepository,
	...
	private async void ResubcriptionTimerOnElapsed(object? _, ElapsedEventArgs args)
	{
		...
		foreach (var (channelId, value) in cloned)
			if (DateTime.Now.AddMinutes(-10) >= value)
			{
				...
				{
					_logger.LogError("Subscription with ID {Id} is null when attempting to resubscribed due to lease",
						channelId);
					return;
				}
	...
	public async Task<Result> UpdateSubscriptionSettingsAsync(string guildId, string? uploadChannel = null,
		string? uploadMessage = null, string? liveChannel = null,
	...
	public async Task<Result<bool>> IsSubscribedAsync(string guildId, string youtubeChannelUrl)
	{
		...
		if (subscription is null)
		{
			_logger.LogError("Subscription with ID {Id} was not found when checking if guild {GuildId} was subscribed",
				youtubeChannelId, guildId);
	...
	public async Task<Result> SubscribeAsync(string youtubeChannelUrl, string guildId)
	{
		...
		if (guild is null || guild.YoutubeUploadLiveChannel is null || guild.YoutubeUploadLiveMessage is null
		    || guild.YoutubeUploadNotificationChannel is null ||
		    guild.YoutubeUploadNotificationMessage is null) return Result.FromError(new UnconfiguredError());

		var (youtubeChannelId, youtubeChannelTitle) =
			await _channelInfoRepository.GetChannelInfoAsync(youtubeChannelUrl);
		...
		if (subscription is not null)
		{
			...
			_logger.LogInformation("Added a subscription to {YoutubeChannelId} for {GuildId}", youtubeChannelId,
				guildId);
		}
		else
		{
	...
	private async Task<Result> RemoveSubscriptionAsync(string guildId, string youtubeChannelId)
	{
		...
		if (subscription is not null)
		{
			...
			if (subscription.GuildIds.Length > 1)
			{
				...
				_logger.LogInformation("removed a subscription to {ChannelId} for {GuildId}", youtubeChannelId,
					guildId);
			}
			else
			{
				...
				_logger.LogInformation(
					"Removing subscription to channel {ChannelId} entirely as no guild are subscribed to it currently",
					youtubeChannelId);
			}
		}
		else
		{
			_logger.LogWarning(
				"Subscription with id {Id} does not exist, yet an attempt was made to unsubscribe from it",
				youtubeChannelId);
	...
	public async Task<Result> UnsubscribeFromAllAsync(string guildId)
	{
		...
		if (!exists)
		{
			_logger.LogWarning("Guild {GuildId} tried to unsubscribe from all but had no subscriptions to begin with",
				guildId);
			return Result.FromSuccess();
		}
	...
	public async Task AddSeenVideoAsync(string youtubeChannelId, string videoId)
	{
		...
		{
			_logger.LogWarning(
				"Subscription to channel with ID {ChannelId} not found when trying to add seen video with ID {VideoId}",
	...
	public async Task UpdateChannelNameAsync(string youtubeChannelTitle, string youtubeChannelId)
	{
		...
		{
			_logger.LogWarning(
				"Could not find subscription for channel ID {Id} with name {Name} when trying to update the name",
				youtubeChannelId, youtubeChannelTitle);
			return;
		}

		if (subscription.ChannelTitle == youtubeChannelTitle)
		{
			_logger.LogTrace("Not updating channel name with id {Id} as it's the same ({Name})", youtubeChannelId,
				youtubeChannelTitle);
			return;
		}
