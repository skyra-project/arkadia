...
public class SubscriptionManagerTests
{
	...
	public async Task SubscriptionManager_IsSubscribed_ReturnsTrue_WhenSubscriptionExists()
	{
		...
		await notificationRepo.AddSubscriptionAsync(youtubeChannelId, DateTime.Now.AddDays(10), new[] { guildId },
			channelTitle);
	...
	public async Task SubscriptionManager_Subscribe_ReturnsUnconfiguredError_WhenGuildIsNotPresent()
	{
		...
		await notificationRepo.AddSubscriptionAsync(youtubeChannelId, DateTime.Now.AddDays(10), Array.Empty<string>(),
			channelTitle);
	...
	public async Task SubscriptionManager_Subscribe_UpdatesDataCorrectly()
	{
		...
		Assert.That(notificationRepo.GetSubscriptions(),
			Has.Exactly(1).EqualTo(expected).Using(new YoutubeSubscriptionComparer()));
	}
	...
	public async Task SubscriptionManager_Unsubscribe_DoesNotRemoveSubscription_WhenMultipleGuildsAreSubscribed()
	{
		...
		await mockRepo.AddSubscriptionAsync(youtubeChannelId, DateTime.UtcNow.AddDays(5),
			new[] { firstGuildId, secondGuildId }, channelName);
	...
	[Test]
	public async Task
		SubscriptionManager_UnsubscribeFromAll_ReturnsChannelInfoRetrievalError_WhenChannelInfoFetchingFails()
	{
		// arrange
	...
	[Test]
	public async Task
		SubscriptionManager_UnsubscribeFromAll_ReturnsChannelInfoRetrievalError_WhenChannelInfoFetchFails()
	{
		// arrange
	...
	public async Task SubscriptionManager_AddSeenVideo_UpdatesCorrectly_WhenNoEntriesExist()
	{
		...
		var manager =
			new SubscriptionManager(null!, new NullLogger<SubscriptionManager>(), mockRepoFactory, null!, null!);
